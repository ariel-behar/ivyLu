...
router.post('/register', async (req, res) => {
    ...
    try {
        let userExists = await clientServices.getOneByEmail(email);
        ...
        if(userExists){
           throw {statusCode: 403, message: "This email address is already being used by another user."}
            ...
            try {
                ...
                if(imgUrl) {
                    userResponse = await clientServices.register({firstName, lastName,email, phone, gender, role, imgUrl, password });
                } else {
                    userResponse = await clientServices.register({firstName, lastName,email, phone, gender, role, password });
                }
...
router.post('/login', async (req, res) => {
    ...
    try {
        let userResponse = await clientServices.login(email)

        if(!userResponse) {
            userResponse = await staffServices.login(email)
        }
